...
suite('Lunor Document Symbols', () => {
  ...
  test('provides component and children symbols', async () => {
    ...
    assert.ok(Array.isArray(root.children), 'root.children should be an array');
    assert.strictEqual(root.children!.length, 3);

    assert.strictEqual(root.children![0].name, 'State count', 'first child should be state');
    assert.match(root.children![1].name, /^for item in items$/, 'second child should be for loop');
    assert.match(root.children![2].name, /^if cond$/, 'third child should be if condition');
  });
});
